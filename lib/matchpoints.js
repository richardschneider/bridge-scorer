...
function matchpoints (games, scoring) {
    ...
    // Add out matchpoints object to each game
    ...
    for (i = 0; i < games.length; ++i) {
        ...
        if (game.contract && game.contract.declaror && game.score) {
            var playedGame = { 
                game: game,
    ...
    while (i <= n)
    {
        ...
        {
            ++i;
            played[i].game.matchpointsNS.value = matchpoints;
            played[i].game.matchpointsEW.value = totalMatchpoints - matchpoints;
        }
        ++i;
    }    
